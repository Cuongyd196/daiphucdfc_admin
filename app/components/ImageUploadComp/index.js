...
export default function ImageComponent({ imageSrc='', width=128, aspect=1, btnTitle='',  onChange  }) {
  const [imgSrc, setImageSrc] = useState(imageSrc);

  const loading = useSelector(makeGetLoading());

  useEffect(() => {
    console.log(imageSrc, 'imageSrcimageSrcimageSrcimageSrc')
    if(imageSrc){
      setImageSrc(API.FILES.format(imageSrc));
    }
  }, [imageSrc]);
  ...
  return <Space direction="vertical">
    ...
    <ImgCrop rotate aspect={aspect}>
      ...
      >
        <Button size="small" disabled={loading}> {btnTitle || 'Thay đổi hình ảnh'}</Button>
      </Upload>
    </ImgCrop>
  </Space>
}
