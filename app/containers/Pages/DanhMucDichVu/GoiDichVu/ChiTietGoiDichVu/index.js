...
import {
  Table, Input, InputNumber, Popconfirm, Form, Statistic, Button, Row, Col, Divider, Modal, Select, Tooltip, message,
...
import { updateById, addDichVu, putDichVu, deleteDichVu, getById } from '@services/danhmucdichvu/goidichvuService';
...
class AddGoiDichVu extends Component {
  ...
  constructor(props) {
    super(props);
    let { match, myPermission } = this.props;
    this.state = {
      page: 1,
      limit: 50,
      totalDocs: 0,
      ...
      _id: match.params.id ? match.params.id : '',
      showModal: false,
    ...
    this.columns = [
      ...
      { title: 'Dịch vụ', dataIndex: 'magiadichvu', width: 200, render: (value) => value.ten },
      {
        title: 'Giá tiền',
        align: 'center',
        editable: true,
      ...
      },
      {
        title: <div>
          {
            myPermission?.['quan-ly-dich-vu']?.sua &&
            <Button size='small' type="primary" ghost onClick={this.toggleModal} icon={<PlusOutlined/>}>
              Thêm dịch vụ
            </Button>
          }
        </div>,
        ...
        width: 150,
        render: (value) => {
          if(!myPermission?.['quan-ly-dich-vu']?.sua) return null
  ...
  async componentDidMount() {
    try {
      let { _id } = this.state;
      if (_id) {
        let [apiRequest, dmgoidichvu, dmdichvu] = await Promise.all([getById(_id), getAllDmDichVu(1, 0), getDmDichVu(1, 0)]);
        if (apiRequest) {
          this.formRef.current.setFieldsValue({...apiRequest, dmgoidichvu_id: apiRequest.dmgoidichvu_id ? apiRequest.dmgoidichvu_id._id : null});
          this.setState({
            imageSrc: apiRequest.avatar ? API.FILES.format(apiRequest.avatar) : null,
            mota: apiRequest.mota,
            list_Selected_DichVu: apiRequest.dichvu,
            dmgoidichvu: dmgoidichvu.docs || [],
            dmdichvu: dmdichvu.docs || []
          });
        }
      }
    } catch (e) {
  ...
  handleAddPatient = async () => {
    let { list_DichVu_Modal, _id } = this.state;
    let list_Selected_DichVu = [];
    list_DichVu_Modal.map((item) => {
      if (item !== undefined) {
        const select_item = { magiadichvu: item._id, giadichvu: item.giadichvu, ghichu: item.ghichu };
        list_Selected_DichVu = [...list_Selected_DichVu, select_item];
      }
    });
    const apiRequest = await addDichVu(_id, list_Selected_DichVu);
    if (apiRequest) {
  ...
  handleDelete = async (value) => {
    try {
      ...
      let { _id } = this.state;
      const apiRequest = await deleteDichVu(_id, value._id);
      if (apiRequest) {
  ...
  save = async (value) => {
    try {
      const row = await this.formTable.current.validateFields();
      row.magiadichvu = value.magiadichvu;
      ...
      let { _id } = this.state;
      const apiRequest = await putDichVu(_id, value._id, row);
      if (apiRequest) {
  ...
  onFinish = async (values) => {
    ...
    const apiResponse = await updateById(this.state._id, values);
    if(apiResponse){
      this.setState({
        imageFile: null,
        imageSrc: apiResponse.avatar ? API.FILES.format(apiResponse.avatar) : null,
      })
  ...
  render() {
    const { loading, dmgia, myPermission } = this.props;
      ...
      <div>
                ...
                <Col xs={24} sm={12} md={6}>
                  <Form.Item label=" ">
                    <Button type="primary" htmlType="submit" icon={<SaveOutlined/>} disabled={loading}
                            className="pull-right">Lưu dữ liệu</Button>
                  </Form.Item>
                </Col>
              }*/}
        ...
        <Modal title='Chọn dịch vụ thêm vào gói'
          ...
          <Form ref={this.formRefModal} id="formRefModal" className='form-no-feedback' name='formRefModal'
                ...
                onFinish={this.onFinishFilter_Modal.bind(this)}>
            ...
            <Row gutter={10}>
              <Col xs={24} md={12}>
                <Form.Item label="Dịch vụ" name="ten" {...layoutFilter}>
                  <Input placeholder='Nhập dịch vụ' disabled={loading}/>
                </Form.Item>
              </Col>
              <Col xs={24} md={12}>
                <Form.Item label="Nhóm dịch vụ" name="dmdichvu_id" {...layoutFilter}>
                  <Select placeholder='Chọn nhóm dịch vụ' disabled={loading} dropdownClassName='small'>
              ...
              <Col xs={24} md={12} className='ml-auto'>
                ...
                <Button htmlType='button' size='small' className='pull-right mr-2' danger
                        icon={<DeleteOutlined/>}
                        onClick={this.clearFilter_Modal.bind(this)}>
                  Xoá bộ lọc
                </Button>
              </Col>
            </Row>
