...
import { add } from '@services/danhmucchung/nhanvienService';
...
class AddNhanVien extends Component {
  constructor(props) {
    super(props);
    this.state = {
      mota: '',
  ...
  onFinish = async value => {
    ...
    const apiResponse = await add(value);
    if (apiResponse) {
      message.success('Thêm dữ liệu thành công');
      this.props.history.push(URL.DANHMUC_NHANVIEN);
    }
  };
  ...
  render() {
    ...
    return <Card
      title={<span><UnorderedListOutlined className="icon-card-header"/> &nbsp; Đăng ký thêm giáo viên</span>}
      size="small" md="24" bordered
      extra={myPermission?.[this.url]?.them &&
...
export default compose(withConnect, withPhai)(AddNhanVien);
